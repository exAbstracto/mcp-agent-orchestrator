...
class MessageQueueServer:
    ...
    def __init__(self, name: str = "message-queue", version: str = "1.0.0"):
        ...
        self.subscriptions: Dict[str, List[Subscription]] = defaultdict(list)  # channel -> subscriptions
        self.agent_subscriptions: Dict[str, Set[str]] = defaultdict(set)  # agent_id -> channels
            ...
            "tools": [
                {
                    ...
                    "inputSchema": {
                        ...
                        "properties": {
                            ...
                            "ttl_seconds": {"type": "number", "description": "Time to live"}
                        },
        ...
        while self._running:
            try:
                ...
                for msg_id in expired_message_ids:
                    ...
                    if message:
                        self.logger.debug(f"Expired message {msg_id} from channel {message.channel}")
                        self.metrics.messages_failed += 1
                        
                # Clean up empty channel queues
                empty_channels = [
                    channel for channel, queue in self.messages.items() 
                    if len(queue) == 0
                ]
    ...
    def _publish_message(self, request_id: Union[str, int, None], 
        ...
        try:
            ...
            return self._create_error_response(request_id, -32603, f"Error publishing message: {e}")
            
    def _subscribe_channel(self, request_id: Union[str, int, None], 
                          arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            existing = any(
                ...
                for sub in self.subscriptions[channel]
            )
            ...
            # Update metrics
            self.metrics.subscribers_count = sum(len(subs) for subs in self.subscriptions.values())
            ...
            return self._create_error_response(request_id, -32603, f"Error subscribing: {e}")
            
    def _unsubscribe_channel(self, request_id: Union[str, int, None], 
                            arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            return self._create_error_response(request_id, -32603, f"Error unsubscribing: {e}")
            
    def _get_messages(self, request_id: Union[str, int, None], 
                     arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.debug(f"Retrieved {len(messages)} messages for agent {agent_id}")
            ...
            return self._create_error_response(request_id, -32603, f"Error getting messages: {e}")
            
    def _acknowledge_message(self, request_id: Union[str, int, None], 
                            arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            if message:
                ...
                self.logger.debug(f"Message {message_id} acknowledged by agent {agent_id}")
            ...
            return self._create_error_response(request_id, -32603, f"Error acknowledging message: {e}")
            
    def _get_performance_metrics(self, request_id: Union[str, int, None], 
                                arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            return self._create_error_response(request_id, -32603, f"Error getting metrics: {e}")
            
    def _list_channels(self, request_id: Union[str, int, None], 
                      arguments: Dict[str, Any]) -> Dict[str, Any]:
        ...
        try:
            ...
            for channel_name in self.messages.keys():
                subscribers = [sub.agent_id for sub in self.subscriptions.get(channel_name, [])]
                ...
                channels.append({
                    ...
                    "created_at": min(
                        (sub.created_at for sub in self.subscriptions.get(channel_name, [])),
                        default=time.time()
                    )
                })
            ...
            return self._create_error_response(request_id, -32603, f"Error listing channels: {e}")
